...
class _SimpleAppUpgradeWidget extends State<SimpleAppUpgradeWidget> {
  ...
  _buildTitle() {
    return Padding(
        padding: EdgeInsets.only(top: 20, bottom: 30), child: Text(widget.title ?? '', style: widget.titleStyle ?? TextStyle(fontSize: 22)));
  }
  ...
  _buildOkActionButton() {
    ...
    if (widget.force) {
      borderRadius = BorderRadius.only(bottomRight: Radius.circular(widget.borderRadius), bottomLeft: Radius.circular(widget.borderRadius));
    }
    ...
    return Ink(
      decoration: BoxDecoration(
          gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [_okBackgroundColors![0], _okBackgroundColors![1]]),
  ...
  Widget _buildDownloadProgress() {
    return widget.progressBar ??
        LiquidLinearProgressIndicator(
          ...
          valueColor: AlwaysStoppedAnimation(widget.progressBarColor ?? Theme.of(context).primaryColor.withOpacity(0.4)),
          borderRadius: widget.borderRadius,
          borderWidth: 1,
          borderColor: Color.fromRGBO(70, 150, 236, 1),
        );
  }
  ...
  _downloadApk(String url, String path) async {
    if (_downloadStatus == DownloadStatus.start || _downloadStatus == DownloadStatus.downloading || _downloadStatus == DownloadStatus.done) {
